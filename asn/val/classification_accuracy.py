...
def accuracy(data_loader, model_forward, criterion,img_keys,lable_keys, n_batch=None, use_cuda=True,writer=None,step_writer=None,task_names=None,plot_name="acc"):
    ''' percent '''
    acc_loss_top_1=[]
    criterion_loss=[]
    ...
    for _ in range(repeat_cnt):
        for batch_iter, sampels in enumerate(data_loader):
            inputs=torch.cat([sampels[k]for k in img_keys])
            targets=torch.cat([sampels[k]for k in lable_keys])
            if use_cuda:
...
def tb_write_class_dist(writer,class_predictions,data_cnt,step_writer,task_names, plot_name):
    ...
    if 0 not in class_predictions:
        log.warn("distri class not starts with lable zero, missing classe in dataloader!")
    # cm=[d/np.mean(d) for c,d in sorted(class_predictions.items())]
