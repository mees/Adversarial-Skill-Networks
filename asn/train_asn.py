...
from asn.utils.train_utils import (
    get_dataloader_train,
    get_dataloader_val,
    get_metric_info_multi_example,
    get_skill_dataloader,
    init_log_tb,
    log_train,
    multi_vid_batch_loss,
...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="cstack", help="real block data: cstack, action dataset: uulmMAD")
    ...
    parser.add_argument("--d-net-hidden-dim", type=int, default=500, help="hidden dim for the discriminator")
    parser.add_argument("--d-net-z-dim", type=int, default=64, help="z dimension for the discriminator")
    parser.add_argument(
        "--multi-domain-frames-stride", type=int, default=15, help="num frames between each sampled frame"
    )
    parser.add_argument(
        "--train-filter-tasks", help="tasks to filter for training: 'taskx,taskb'", type=str, default=None
    )
    parser.add_argument(
        "--num-example-batch", help="num frames from different pairs sampled for a batch", type=int, default=4
    )
...
def main():
    ...
    log.info("criterion: for {} ".format(criterion.__class__.__name__))

    asn, global_step_start, _, _ = create_model(use_cuda, args.load_model, embedding_size=args.emb_dim)
    ...
    dataloader_train_domain = get_skill_dataloader(
        args.train_dir,
        args.num_views,
        args.batch_size,
        use_cuda,
    ...
    for global_step in range(global_step_start, args.steps):
        ...
        if examples_per_seq == 1:
            ...
            loss_metric = multi_vid_batch_loss(criterion, embeddings, labels, num_vid_example=examples_per_seq)
        ...
        entropy_margin = -1.0 * marginalized_entropy(d_out_gen)
        # ensure equal usage of fake samples
        ...
        entropy_real = -1.0 * entropy(d_out_gen)
        ...
        if global_step % args.val_step == 0 and global_step > global_step_start:
            ...
            loss_val, nn_dist, dist_view_pais, frame_distribution_err_cnt = view_pair_alignment_loss(
                model_forward_np, args.num_views, dataloader_val
            )
            asn.train()

            writer.add_histogram("val/frame_error_count", np.array(frame_distribution_err_cnt), global_step)
            writer.add_scalar("val/alignment_loss", loss_val, global_step)
            writer.add_scalar("val/nn_distance", nn_dist, global_step)
            writer.add_scalar("val/distance_view_pairs_same_frame", dist_view_pais, global_step)
            ...
            log.info(msg)
            save_model(asn, optimizer_g, args, is_best, args.save_folder, global_step)
