...
def get_args():
    ...
    parser.add_argument("--output-vid-name", type=str, default="out1.mov")
    parser.add_argument("--mun-col", help="vids in x", type=int, default=4)
    parser.add_argument("--mun-row", help="vids in y", type=int, default=5)
    parser.add_argument("--view-idx", help="view to show", type=int, default=0)
    parser.add_argument("--num-views", help="number of views", type=int, default=2)
    ...
    parser.add_argument("--fps", help="out vid fps", type=int, default=24)
    parser.add_argument(
...
def get_frames(input_vid_file, n_process, num_frames, image_size):
    ...
    for p_ranke in range(n_process):
        ...
        args_worker = (p_ranke, num_frames, image_size, input_frames_q, result_frames_q)
...
def main():
    ...
    vid_writer = None
    for frames in tqdm(
        get_frames(input_imgs_file, args.mun_col * args.mun_row, args.num_frames, image_size),
        desc="frame",
        total=args.num_frames,
    ):

        imgs = [[frames[y] for y in range(x, x + args.mun_row)] for x in range(0, len(frames), args.mun_row)]
        ...
        if vid_writer is None:
            ...
            vid_writer = cv2.VideoWriter(args.output_vid_name, fourcc, fps, out_shape_cv)
