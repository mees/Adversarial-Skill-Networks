...
def get_args():
    ...
    parser.add_argument("--output-img-name", type=str, default="frame_sequence_out.png")
    parser.add_argument("--img-height", help="vid output height", type=int, default=100)
    parser.add_argument("--num-views", help="number of views", type=int, default=2)
    parser.add_argument("--img-width", help="vid output width", type=int, default=100)
    parser.add_argument("--n-frame", help="erver n frame", type=int, default=4)
    parser.add_argument(
...
def show_sequence(imgs, delay=0, n_frame=1, save_name=None, to_rgb=True):
    ...
    titles = [["frame {}".format(n * n_frame) for n in range(len(i))] for i in imgs]
    # take one the longest title row to show on top
    titles = [sorted(titles, key=len, reverse=True)[0]]
    montage_image = montage(imgs, titles=titles, margin_separate_vertical=0, margin_separate_horizontal=5)
...
def main():
    ...
    for f in input_imgs_file:
        imgs_vid = VideoFrameSampler(f, dtype=dt, resize_shape=image_size, to_rgb=False).get_all()
        total_frames = len(imgs_vid)
        log.info("file {} with frames: {}".format(f, total_frames))
        imgs.append(imgs_vid)

    show_sequence(imgs, delay=0, n_frame=n_frame, save_name=args.output_img_name, to_rgb=False)
