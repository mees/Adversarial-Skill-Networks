...
def montage(imgs=[[]], margin_top=20, margin_bottom=20,
            ...
            margin_seperat_vertical=20, margin_seperat_horizontal=20,
            margin_color_bgr=[1, 1, 1],
            titels=None, fontScale=0.5, titel_pos=None):
    ''' creats a montage image with imgs in a grid layout
    Args:
        imgs: 2d list with numpy images with same shapes
        titels: 2d list with strings used das a title at the top of a img,
                must have same shape as imgs
        titel_pos: postion in img with margins
    ...
    usage:
        ...
        imgs_concat=montage(imgs,titels=titels,margin_color_bgr=[0.5,0.5,0.5])
    ...
    for v_index, i_v in enumerate(imgs):
        ...
        for h_index, i_h in enumerate(i_v):
            ...
            m_top = margin_seperat_horizontal if v_index != 0 else margin_top
            m_bot = 0 if v_index != len(imgs) - 1 else margin_bottom
            m_l = margin_seperat_vertical if h_index != 0 else margin_left
            ...
            if titels is not None:
                text = ""
                if v_index < len(titels) and h_index < len(titels[v_index]):
                    text = titels[v_index][h_index]
                titel_font = cv2.FONT_HERSHEY_SIMPLEX
                fontColor = (0, 0, 0)  # black
                thickness = 1
                text_size, _ = cv2.getTextSize(
                    text, titel_font, fontScale, thickness)
                # title postion at the top of img
                if titel_pos is None:
                    titel_pos_top = (m_l, max(
                        m_top - text_size[1], text_size[1]))
                cv2.putText(i, text, titel_pos_top,
                            titel_font, fontScale, fontColor, thickness)
            h_imgs.append(i)
...
if __name__ == '__main__':
    ...
    # titels = [["black", "white", "black"]]
    # imgs = [[black, white, black]]
    imgs_concat = montage(imgs, titels=titels,
