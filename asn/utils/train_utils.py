...
def combi_push_stack_color_to_task(vid_file_comm, frame_idx=None, vid_len=None, csv_file=None, state_lable=None):
    """ return task label for  push stack color """
...
def uulmMAD_file_name_to_task(vid_file_comm, frame_idx=None, vid_len=None, csv_file=None, state_lable=None):
    ...
    assert act in [
        ...
        "ST3",
        "ST4",
    ], "act not found {}".format(act)
    return act


def transform_vid_name_to_task(dataset):
    """ return func to transform vid name to task label name"""
    return {"cstack": combi_push_stack_color_to_task, "uulmMAD": uulmMAD_file_name_to_task}[dataset]
...
def get_dataloader_train(
    dir_vids, num_views, batch_size, use_cuda, img_size=299, filter_func=None, label_funcs=None, examples_per_seq=None
):
...
def get_dataloader_val(dir_vids, num_views, batch_size, use_cuda, filter_func=None, img_size=299):
    ...
    dataset_val = ViewPairDataset(
        vid_dir=dir_vids, number_views=num_views, filter_func=filter_func, transform_frames=transformer_val
    )

    dataloader_val = DataLoader(
        dataset_val, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=use_cuda  # * 2,
    )

    return dataloader_val


def get_skill_dataloader(
    dir_vids, num_views, batch_size, use_cuda, img_size, filter_func, label_funcs, num_domain_frames=1, stride=1
):
...
def get_metric_info(anchor_emb, positive_emb):
    ...
    info_metric["dist pos"] = np.linalg.norm(anchor_emb - positive_emb, axis=1).mean()
    ...
    cnt, d_neg, dots_neg, cosin_neg_dist_neg = 0.0, 0.0, 0.0, 0.0
    # TODO rm this loop
    for i in range(n):
        for j in range(n):
            if j != i:
                ...
                d_neg += d_negative
                dots_neg += np.dot(emb_pos[i] - emb_pos[i].mean(), emb_neg[j] - emb_neg[j].mean())
...
def get_metric_info_multi_example(anchor_emb, positive_emb):
    """ get separate info for each vid in batch """
