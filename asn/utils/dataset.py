...
def get_negative_frame_index(anchor_index, video_length, margin=0):
    """ return frame range option excluding the positive frame """
...
def get_positive_frame_index(anchor_index, video_length, margin=0):
    """ return frame range option around the positive frame """
...
class DoubleViewPairDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            std_similar_frame_margin_distribution: positive frame variable:
                postiive frame ist sampled around the anchor frame with a normal distribution
            filter_func: function with two inputs: common name and list with frames number,
                        returns true if video is ok
            dict label_funcs: dict with tag in key and function to add a user label,
    ...
    def __getitem__(self, idx):
        ...
        self._update_user_labels(samples, vid_file_comm, frame_index_anchor, view_min_frame_len, csv_file)
        return samples

    def _update_user_labels(self, samples, vid_file_comm, frame, vid_len, csv_file):
...
class ViewPairDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample. if None no transformation
            filter_func: function to filter videos, input common view pair name, and view pairs frame len list
        """
        self.transform_frames = transform_frames
        self.n_views = number_views
        self.vid_dir = vid_dir
